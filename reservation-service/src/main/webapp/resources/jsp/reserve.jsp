...
<body>
    <div id="container">
        ...
        <div class="header fade">
            <header class="header_tit" data-id="${product.id}">
                <h1 class="logo">
                        ...
                        <ul class="visual_img">
                            <li class="item" style="width: 414px;"> <img alt="" class="img_thumb" src="/files/${image.fileId }"> <span class="img_bg"></span>
                                <div class="preview_txt">
                                    <h2 class="preview_txt_tit">${product.name }</h2> <em class="preview_txt_dsc">${product.description}</em><em class="preview_txt_dsc">${product.description}</em> </div>
                            </li>
                        </ul>
                    </div>
                </div>
	                    ...
	                    <c:forEach var="price" items="${priceList }">
	                   		<c:choose>
		                   		<c:when test="${price.priceType eq 1 }">
		                   			<c:set var="age" value="성인"/>
		                   		</c:when>
		                   		<c:when test="${price.priceType eq 2 }">
		                   			<c:set var="age" value="청소년"/>
		                   		</c:when>
		                   		<c:when test="${price.priceType eq 3 }">
		                   			<c:set var="age" value="어린이"/>
		                   		</c:when>
	                   		</c:choose>

	                        <div class="qty" data-index="${num}">
                                ...
                                <span data-type="${price.priceType}">${age }</span> </strong>
                                <strong class="product_price"> <span class="price" data-discount="${price.discountRate}"><fmt:formatNumber value="${price.price}" pattern="#,###"/></span> <span class="price_type">원</span> </strong>
                                <em class="product_dsc"><span><fmt:formatNumber value="${price.price*(1-price.discountRate)}" pattern="#,###"/></span>원<span> (<fmt:formatNumber value="${price.discountRate*100}" pattern="##"/>% 할인가)</span></em>
                              </div>
	                        </div>
                                ...
                                <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                                    <div class="inline_control">
                                        <p class="inline_txt selected">
                                          <span>총</span><span>0</span><span>매</span>
                                        </p>
                                    </div>
                                </div>
                                <c:forEach var="price" items="${priceList }">
